...
The goal of this vignette is to demonstrate the workflow for simulating a habitat loss scenario using rcoalescence and estimating biodiversity losses in different areas of the landscape. It is an extended version of the vignette found within the `rcoalescence` package itself, aimed at supporting Thompson *et al* (2020).
...
knitr::kable(mean_species_richness %>%
  rename(
    `community reference` = community_reference,
    `species richness` = mean_species_richness,
  ))
```
...
species_richness_plot <- mean_species_richness %>% ggplot() +
  geom_ribbon(aes(x = 100 - time, ymin = min_species_richness, ymax = max_species_richness), alpha = 0.6) +
  geom_line(aes(
    x = 100 - time,
...
```{r warning=FALSE, message=FALSE, fig.fullwidth=TRUE, fig.width=6, fig.cap="Species abundance distributions (Preston plots) for the current and future landscapes."}
sad_plot <- mean_species_abundances %>%
  filter(time == 0 | time == 100) %>%
  mutate(scenario = ifelse(time == 0, "Future landscape", "Current landscape")) %>%
  ggplot() +
  geom_ribbon(aes(x = log_abundance, ymin = min_species, ymax = max_species), alpha = 0.5) +
...
species_density_plot <- mean_species_density %>%
  ...
  scale_fill_viridis("Mean species richness") +
  theme_void() +
...
national_park_loss_gain_plot <- species_loss_gain %>%
  ...
  scale_fill_distiller("Species\nrichness\nloss/gain",
    palette = "Spectral",
    direction = 1,
    limits = c(-1, 1) * 0.7 * max(abs(national_park_species_density$mean_species_richness), na.rm = TRUE)
  ) +
  theme_void() +
  scale_y_reverse() +
...
column_2 <- ggarrange(
  ...
  labels = c(
    "c) Mean species richness over time",
